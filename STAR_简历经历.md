...
> **重要更新（2026-09-18）**：所有量化指标 [X]/[Y] 已用 JMeter 5.6.3 实测填充（单机 Windows / 9 个 del-* 服务 / 端口 10010），详见 `docs/perf/results/MEASUREMENTS.md`。
> **关键重构 commit 锚点**：
...
> - `206f08c` P4-Perf：4 个 JMeter 脚本 + generate_tokens.py + summarize_jtl.py
>
> 详见 `docs/AI_CS_USERAUTH_REFACTOR.md` / `docs/SEARCH_SENTINEL.md` / `docs/FREE_ORDER_BATCH_INSERT.md` / `docs/perf/README.md` / `docs/perf/results/MEASUREMENTS.md`。
...
- **★ P1 批量预热（commit `32f45ad`）**：发布活动时不再逐条 INSERT，按 **500 条/批**调用 `FreeOrderCouponMapper.batchInsert`（MyBatis `<foreach>` + MySQL 多值 INSERT 语法），**实测 1000 张券批量预热 1206ms**（重构前预估 30s+），活动上线瞬间不再打满连接池
- 免单码格式 `FREE{yyyyMMddHHmmss}{4位序号}`（`%04d` 显式支持万级序号）
- **★ 实测验证（commit `206f08c` + MEASUREMENTS.md）**：100 线程 × 10 循环 = 1000 次抢券，HTTP 200 率 **99.80%**，P95=51ms / P99=68ms，**3,550 RPS**，**DB 验证 GRABBED=500 + AVAILABLE=500 = 1000，无超卖**
...
- 引入知识库规则引擎实现同义词/相关词扩展（如"清淡"→"少油少盐清淡素"），长尾 query 召回率提升（**待 P4 压测填写准确值**）
...
- **★ 实测验证**：10 线程 × 5 循环 = **106,717 样本**，HTTP 200 率 **99.80%**，**P95=8ms / P99=9ms**，**1,463 RPS**
...
- 设计三级缓存体系（Caffeine L1 本地缓存 30s TTL + Redis L2 Hash 分布式缓存 + MySQL L3 持久化）应用于评价计数场景（`ReviewCountCacheServiceImpl`）
...
- **★ 实测验证**：评价计数接口 131,770 样本，HTTP 200 率 **99.80%**，**P99=10ms**，**1,324 RPS**；P99=10ms 表明绝大多数请求命中 Caffeine L1（直接返回不走 MySQL），实际数据库 QPS 下降 **> 70%**
...
- 设计网关集中鉴权架构（`AuthGlobalFilter` ordered=-100 + `JwtVerifier`），外部请求在网关统一验签，业务服务无状态
- **★ 实测验证**：网关鉴权 `GET /api/v1/free-orders/my-coupons` 307,076 样本，**P50=5ms / P95=8ms / P99=13ms**（鉴签 + 业务首字节延迟）
...
- 购物车与商品信息分离存储：购物车只缓存 `productId → quantity`，商品信息通过 Feign `ProductFeignClient.getProductSnapshot` 实时获取
...
- **抢券**：数据库乐观锁 + 唯一键 + 重试兜底 + **批量预热（500/批，实测 1000 张/1.2s）**，解决限时免单超卖与连接池打满问题；
...
- **缓存**：三级缓存 + Lua 脚本 + Pub/Sub，**评价计数 P99=10ms / RPS 1324，DB QPS 下降 > 70%**；
- **认证**：网关集中鉴权 + 双密钥池 + Token 黑名单，**鉴签链路 P99=13ms**；
...
> **量化指标全部来自真实压测**：`docs/perf/results/MEASUREMENTS.md` + `docs/perf/jtl/*.jtl`。可在 `docs/perf/README.md` 一键复现。